...
from evennia import AttributeProperty, search_object, create_object
from evennia.utils.utils import make_iter

from . import rules
from .rules import damage_engine
...
class QuestObject(Object, DefaultObject):
    """
...
class Weapon(Object, DefaultObject):
    """Base class for all weapons"""

    obj_type = ObjType.WEAPON
    inventory_use_slot = AttributeProperty(WieldLocation.MAIN_HAND, autocreate=False)

    attack_type = AttributeProperty(Ability.STR, autocreate=False)
    defense_type = AttributeProperty(Ability.ARMOR, autocreate=False)

    damage_roll = AttributeProperty("4-6", autocreate=False)


    def at_pre_use(self, user, target=None, *args, **kwargs):
        if target and user.location != target.location:
            # we assume weapons can only be used in the same location
            user.msg("You are not in the same location as the target!")
            return False
    ...
    def use(self, attacker, target, *args, advantage=False, disadvantage=False, **kwargs):
        ...
        if is_hit:
            ...
            dmg = damage_engine.damage(self.damage_roll)
...
class Armor(Object, DefaultObject):
...
class EvAdventureShield(Armor):
    obj_type = ObjType.SHIELD
    inventory_use_slot = WieldLocation.OFF_HAND


class EvAdventureHelmet(Armor):
    obj_type = ObjType.HELMET
    inventory_use_slot = WieldLocation.HEAD


class WeaponBareHands(Weapon):
    obj_type = ObjType.WEAPON
    inventory_use_slot = WieldLocation.MAIN_HAND
